...
wrf2df_theta <- function(file_t, file_znu, poi, tz) {
    # Get the netcdf ID's for the temperature and the heights
...
wrf2df_metwind <- function(file_wind, file_t, file_znu, poi, tz, u_v) {
    # Get the netcdf ID's for the temperature and the heights
...
wrf2df_vertwind <- function(file_wind, file_znw, poi, tz) {
    # Get the netcdf ID's for the temperature and the heights
    # u_v  which component of the wind to get, 1 for u, 2 for v
    ncid = nc_open(file_wind) # Dimensions are (XLAT, XLON, level, time)
...
wrfscalar_hw2018 = function(file_t, file_q, file_znu, poi, tz, targethts) {
    # This function is a wrapper fo rcreating WRF potential temperature and water vapor
    # mixing ratio.
    
    # Extract the weather variables
    nc.theta = wrf2df_theta(file_t, file_znu, poi, tz)
    nc.qmix = wrf2df_qmix(file_q, file_znu, poi, tz)
...
wrfwinds_hw2018 = function(file_wind, file_vertwind, file_t, file_znu, file_znw, poi, tz, targethts) {
    ...
    nc.uwind = wrf2df_metwind(file_wind, file_t, file_znu, poi, tz, 1)
    nc.vwind = wrf2df_metwind(file_wind, file_t, file_znu, poi, tz, 2)
    nc.wwind = wrf2df_vertwind(file_vertwind, file_znw, poi, tz)
